...
{
    public partial class AnalysisVarPoint : Form
    {
        public string selectrowid = "";//一个全局变量，用于存储修改配置信息的选择行
        public int selectrow = 0;

        static DataTable dt = new DataTable();
        public AnalysisVarPoint()
        {
            InitializeComponent();
        ...
        {
            string sql = "select * from turbine_var_point where turbineid='" +turbineid + "' and datatime between '" + starttime + "'  and '" + endtime + "' ";
            if (starttime.Equals("") || endtime.Equals("") || turbineid.Equals(""))
            {
                sql = "select * from turbine_var_point";
            }
            DataSet ds_var = MYSQLHelper.GetDataSet(MYSQLHelper.connectionStringManager, sql, null);
        ...
        private void 查看监测变量分析结果ToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string filename = "机组【" + selectrowid + "】【" + DateTime.Parse(this.dateTimePicker1.Text).ToString("yyyy-MM-dd") + "】【" + DateTime.Parse(this.dateTimePicker2.Text).ToString("yyyy-MM-dd") + "】" + DateTime.Now.ToString("yyyyMMddHHmmss") + "变量分析结果";
