...
namespace TurbineMonitoringVarableAnalysis.Forms
{
    public partial class PointView : Form
    {
        ...
        {
            string sql = "select * from turbine_point where turbineid='" + turbineid + "' and datatime between '" + starttime + "'  and '" + endtime + "' ";
            if (starttime.Equals("") || endtime.Equals("") || turbineid.Equals(""))
            {
                sql = "select * from turbine_point";
            }
            DataSet ds_var = MYSQLHelper.GetDataSet(MYSQLHelper.connectionStringManager, sql, null);
        ...
        private void 查看变量分析结果ToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string filename = "机组【" + selectrowid + "】【" + DateTime.Parse(this.dateTimePicker1.Text).ToString("yyyy-MM-dd") + "】【" + DateTime.Parse(this.dateTimePicker2.Text).ToString("yyyy-MM-dd") + "】" + DateTime.Now.ToString("yyyyMMddHHmmss") + "每时刻分析结果";
