...
{
    public partial class TurbineFaultView : Form
    {
        public string selectrowid = "";//一个全局变量，用于存储修改配置信息的选择行
        public int selectrow = 0;

        static DataTable dt = new DataTable();
        public TurbineFaultView()
        {
            InitializeComponent();
        ...
        {
            string sql = "select * from turbine_fault_rate where turbineid='" + turbineid + "' and datatime between '" + starttime + "'  and '" + endtime + "' ";
            if (starttime.Equals("") || endtime.Equals("") || turbineid.Equals(""))
            {
                sql = "select * from turbine_fault_rate";
            }
            DataSet ds_var = MYSQLHelper.GetDataSet(MYSQLHelper.connectionStringManager, sql, null);
        ...
        private void 查看机组异常率ToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string filename = "机组【" + selectrowid + "】【" + DateTime.Parse(this.dateTimePicker1.Text).ToString("yyyy-MM-dd") + "】【" + DateTime.Parse(this.dateTimePicker2.Text).ToString("yyyy-MM-dd") + "】" + DateTime.Now.ToString("yyyyMMddHHmmss") + "整机异常率";
